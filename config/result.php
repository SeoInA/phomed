...
$Query = "SELECT institutionID FROM (SELECT * FROM (SELECT * FROM daySchedule LEFT JOIN schedule USING(scheduleID) where monday = monday) as SQLbusinessHours JOIN (SELECT * FROM hospital_subject where subject = $subject) as SQLsubject USING(institutionID)) as SQLScheSub JOIN (SELECT * FROM (SELECT * FROM city where city_name = $city_name) as SQLcity JOIN (SELECT * FROM doctor where gender = $gender) as SQLgender USING(institutionID)) as SQLCityGen USING(institutionID)";
